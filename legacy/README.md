...
- `install/` — the PowerShell installer, the desktop-seam builder, the staged app swap,
  ...
  **Removed from the repository** (recoverable with
  `git show 3d5042e:install/install.ps1` and friends): a catalog-listed plugin must not
  ship machinery that fetches and replaces its own files, and none of it is needed next
  to the v13 package.
...
The vendored seam sources under `desktop-patch/seam/` are copies of files from
[NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) (MIT,
Copyright (c) 2025 Nous Research), kept here with their original license.
